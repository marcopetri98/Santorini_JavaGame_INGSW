...
public class PrometheusTest {
	...
	public void checkMoveTestGeneral() throws NoMoveException {
		...
		assertEquals(3, prometheus.checkMove(map, player.getWorker1(), turn).size());

		i=0; j=0;
		Move newMove3 = new Move(TypeMove.SIMPLE_MOVE, map.getCell(x, y), map.getCell(i, j), player.getWorker1());
		assertTrue(prometheus.checkMove(map, player.getWorker1(), turn).contains(newMove3));

		i=0; j=2;
		Move newMove4 = new Move(TypeMove.SIMPLE_MOVE, map.getCell(x, y), map.getCell(i, j), player.getWorker1());
		assertTrue(prometheus.checkMove(map, player.getWorker1(), turn).contains(newMove4));

		i=1; j=0;
		Move newMove5 = new Move(TypeMove.SIMPLE_MOVE, map.getCell(x, y), map.getCell(i, j), player.getWorker1());
		assertTrue(prometheus.checkMove(map, player.getWorker1(), turn).contains(newMove5));
	...
	public void checkBuildTestGeneral() throws NoBuildException {
		...
		assertEquals(3, prometheus.checkBuild(map, player.getWorker1(), turn).size());

		i=0; j=0;
		Build newBuild = new Build(player.getWorker1(), map.getCell(i, j), false, TypeBuild.SIMPLE_BUILD);
		assertTrue(prometheus.checkBuild(map, player.getWorker1(), turn).contains(newBuild));

		i=1; j=0;
		Build newBuild1= new Build(player.getWorker1(), map.getCell(i, j), true, TypeBuild.SIMPLE_BUILD);
		assertTrue(prometheus.checkBuild(map, player.getWorker1(), turn).contains(newBuild1));

		i=0; j=2;
		Build newBuild2 = new Build(player.getWorker1(), map.getCell(i, j), false, TypeBuild.SIMPLE_BUILD);
		assertTrue(prometheus.checkBuild(map, player.getWorker1(), turn).contains(newBuild2));
